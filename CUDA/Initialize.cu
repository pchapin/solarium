...
static float random_position_coordinate( )
{
...
static float random_velocity_component( )
{
    float fraction = (float)( rand( ) ) / RAND_MAX;
    int   sign     = rand( ) % 2;
...
void initialize_object_arrays( )
{
    ...
    // The sun has ID 0.
    object_array[0].mass = 1.98892E+30F;  // Mass of the sun.

    for( int i = 1; i < OBJECT_COUNT; ++i ) {
        object_array[i].mass = 5.9722E+24F;   // Mass of the Earth.
    }
